...
export function OcrAnalysisCard({ ocrData }: OcrAnalysisCardProps) {
  ...
  return (
    <div className="p-6 rounded-xl bg-gradient-to-br from-[#0f1729]/50 to-[#1a1f3a]/50 border border-cyan-500/20 backdrop-blur-sm shadow-[0_0_20px_rgba(34,211,238,0.1)]">
      ...
      {!hasHighRiskSignals && hasWarningSignals && (
        <div className="mt-6 p-4 rounded-lg bg-yellow-500/10 border border-yellow-500/30 flex items-start gap-3">
          <AlertTriangle className="size-5 text-yellow-400 flex-shrink-0 mt-0.5" />
          <div>
            <p className="font-medium text-yellow-300 mb-1">Caution Advised</p>
            <p className="text-sm text-gray-400">
              Some warning signals detected in the text content. Please verify information from official sources.
            </p>
          </div>
        </div>
      )}

      {!hasHighRiskSignals && !hasWarningSignals && signals.length > 0 && (
        <div className="mt-6 p-4 rounded-lg bg-blue-500/10 border border-blue-500/30 flex items-start gap-3">
          ...
          <div>
            <p className="font-medium text-blue-300 mb-1">Low Risk Content</p>
            <p className="text-sm text-gray-400">
              No significant threat indicators found in the extracted text.
            </p>
          </div>
        </div>
      )}
